      ...
      >
        <div className="h-full px-4 pb-4 overflow-y-auto bg-white">
          <ul className="space-y-4">
            ...
            {prompts.map((prompt, index) => (
              <li key={prompt.id} className={`group flex flex-col items-start px-4 py-3 rounded-lg cursor-pointer hover:bg-gray-100 transition-colors duration-300 ${index === 0 ? 'border-t border-gray-200' : ''}`}>
                <button
                  onClick={() => handlePromptClick(prompt)}
                  className="flex items-center space-x-3 w-full text-base font-medium text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                >
                ...
                <select
                  className="mt-2 w-full border rounded-md py-1 px-2"
